...
class ArticleListItem(Item):
    ...
    爬取单页面的内容
    """
    target_item = TextField(css_select="div.mdui-container.mdui-typo.mdui-m-t-2 > div.mdui-typo")
    title = TextField(css_select='div > h3 > a')
    href = AttrField(css_select='div> h3 > a', attr='href')
    # 可能要保留的文本 有的并没有这个
    hint_text = TextField(css_select='div > div.mdui-typo > p',default="")

class IdataSpider(Spider):
    """
    针对搜索引擎: https://search.cn-ki.net/search?keyword=的爬虫
    问答库的
    """
    # 设置启动URL
    start_urls = []
...
async def main(search_text,loop):
    ...
    for page in range(1, 6):
        print('start')
        url = "https://search.cn-ki.net/search?keyword="+quote(search_text)+"&db=CFLS&p="+str(page)
        print(url)
        await IdataSpider.async_start(middleware=middleware, loop=loop, url=url, page=1)
        break
    print('end')
