...
class Item(metaclass=ItemMeta):
    ...
    async def get_multiitems(
        ...
        **kwargs,
        ...
        if html_etree is None:
            html_etree = await cls._get_html(html, url, **kwargs)
        # 对同级的每个field字段都进行提取和解析
        fields_dict = getattr(cls, "__fields", {})
        results = {}
        for field_name, field_value in fields_dict.items():
            # 提取多个
            field_value.many = True
            items_html_etree = field_value.extract(html_etree=html_etree)
            # 得到结果，进行包装返回
            results[field_name] = []
            if items_html_etree:
                for each_html_etree in items_html_etree:
                    results[field_name].append(each_html_etree)
        # results得到字典结果
        return results
